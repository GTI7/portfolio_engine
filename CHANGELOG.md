...
## [1.3.0] — integration 1.2.0 → 1.3.0 / engine unchanged (1.0.0) — Milestone 13: Dashboard & User Experience (2026-07-12)
...
- `docs/testing/MILESTONE_13_MANUAL_VALIDATION.md` — the real-instance validation checklist for this release, run against an installed HACS release rather than gating it beforehand (the project installs from GitHub Releases, not `main` — this release is the prerequisite for that validation, not a consequence of it).
- 6 new tests: 5 pure-logic (`tests_integration/test_sensor_mapping.py` — day-change weighting, allocation grouping/sorting, empty-portfolio edge case), 1 real-HA-harness (`tests_ha/test_setup_and_entities.py` — allocation entity's full attribute shape; both new entities' state/unit also folded into that file's existing `EXPECTED_ENTITIES` table, extending its unique-id-stability and shared-device coverage to them for free) — 484 tests total (up from 478 at the previous release): 332 engine, 50 pure-logic integration, 102 real-HA-harness. (No automated coverage exists or is expected for the dashboard YAML itself — verified by a throwaway Jinja dry-run render against mocked two-portfolio data during development. Real-instance validation, per `docs/testing/MILESTONE_13_MANUAL_VALIDATION.md`'s checklist, follows this release rather than gating it — the project installs via HACS from GitHub Releases, so a release is the prerequisite for that validation, not a consequence of it.)
